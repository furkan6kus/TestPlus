...
public class KagitOkuActivity extends AppCompatActivity {
    ...
    private MaterialCardView cardRectifiedPreview;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        cardRectifiedPreview = findViewById(R.id.cardRectifiedPreview);
    ...
    /**
     * Bitmap en-boyunu formun PDF oranıyla hizalar: dikey formda yatay fotoğraf veya
     * yatay formda dikey fotoğraf ise 90° döndürür (tek taraflı kontrol eksikti).
     */
    private Bitmap alignBitmapOrientationToForm(Bitmap bmp, int pdfW, int pdfH) {
        if (bmp == null || pdfW <= 0 || pdfH <= 0) return bmp;
        boolean formPortrait = pdfH > pdfW;
        boolean bmpLandscape = bmp.getWidth() > bmp.getHeight();
        boolean mismatch = (formPortrait && bmpLandscape)
            || (!formPortrait && !bmpLandscape);
        if (!mismatch) return bmp;
        Log.i("OMR", "Yön düzeltiliyor: form " + (formPortrait ? "dikey" : "yatay")
            + ", fotoğraf " + (bmpLandscape ? "yatay" : "dikey") + " → 90°");
        Matrix m = new Matrix();
        m.postRotate(90);
    ...
    private void launchScanPreview(Bitmap rawBmp) {
        ...
        if (cachedForm != null) {
            ...
            bmp = alignBitmapOrientationToForm(bmp, pdfW, pdfH);
        }
        ...
        intent.putExtra(ScanPreviewActivity.EXTRA_PDF_W_PT, pdfW);
        intent.putExtra(ScanPreviewActivity.EXTRA_PDF_H_PT, pdfH);
        runOnUiThread(() -> {
            if (alignmentOverlay != null) alignmentOverlay.setProcessingText(null);
    ...
    private void processBitmap(Bitmap bitmap) {
        ...
        try {
            ...
            // Form dikey/yatay ile fotoğraf en-boyunu hizala (CameraX / EXIF).
            bitmap = alignBitmapOrientationToForm(bitmap, pdfW, pdfH);
            ...
            runOnUiThread(() -> {
                ...
                if (omrOutcome.rectifiedBitmap != null) {
                    ...
                    if (cardRectifiedPreview != null) {
                        cardRectifiedPreview.setVisibility(View.VISIBLE);
                    }
                    ...
                    if (cardRectifiedPreview != null) {
                        cardRectifiedPreview.setVisibility(View.GONE);
                    }
