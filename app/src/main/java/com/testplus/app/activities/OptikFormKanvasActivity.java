...
import com.testplus.app.utils.GalleryBitmapSaver;
...
public class OptikFormKanvasActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        tvGaleri.setOnClickListener(v -> exportToGallery());
    ...
    /** Kanvas üzerindeki güncel konumları liste içindeki {@link OptikFormAlan} örneklerine yazar. */
    private void applyCurrentViewPositionsToAlanlar(List<OptikFormAlan> alanlar) {
        for (int i = 0; i < kanvasLayout.getChildCount(); i++) {
            View v = kanvasLayout.getChildAt(i);
            if (v instanceof IsaretlemeAlanView) {
                IsaretlemeAlanView iav = (IsaretlemeAlanView) v;
    ...
    private void exportPdf() {
        executor.execute(() -> {
            ...
            List<OptikFormAlan> alanlar = db.optikFormAlanDao().getByFormId(formId);
            applyCurrentViewPositionsToAlanlar(alanlar);
    ...
    /** PDF ile aynı görünümü PNG olarak Resimler/TestPlus altına kaydeder. */
    private void exportToGallery() {
        executor.execute(() -> {
            if (optikForm == null) return;
            List<OptikFormAlan> alanlar = db.optikFormAlanDao().getByFormId(formId);
            applyCurrentViewPositionsToAlanlar(alanlar);
            Bitmap bmp = PdfGenerator.renderFormToBitmap(optikForm, alanlar, 2400);
            final boolean ok = bmp != null && GalleryBitmapSaver.savePngToGallery(
                getContentResolver(), bmp, optikForm.ad != null ? optikForm.ad : "optik_form");
            if (bmp != null) bmp.recycle();
            runOnUiThread(() -> Toast.makeText(this,
                ok ? "Galeriye kaydedildi (Resimler → TestPlus)"
                   : "Görsel oluşturulamadı veya kaydedilemedi",
