...
public class PdfGenerator {
    ...
    /**
     * PDF sayfasıyla aynı vektör içeriği yüksek çözünürlüklü bitmap olarak çizer (PNG / galeri).
     * @param maxSidePx uzun kenar piksel üst sınırı
     */
    public static Bitmap renderFormToBitmap(OptikForm form, List<OptikFormAlan> alanlar, int maxSidePx) {
        if (form == null || alanlar == null || maxSidePx < 120) return null;
        int pdfW = getPdfWidth(form);
        int pdfH = getPdfHeight(form);
        float s = Math.min(maxSidePx / (float) pdfW, maxSidePx / (float) pdfH);
        int outW = Math.max(2, Math.round(pdfW * s));
        int outH = Math.max(2, Math.round(pdfH * s));
        Bitmap bmp = Bitmap.createBitmap(outW, outH, Bitmap.Config.ARGB_8888);
        Canvas c = new Canvas(bmp);
        c.scale(outW / (float) pdfW, outH / (float) pdfH);
        drawFormPage(c, form, alanlar, pdfW, pdfH);
        return bmp;
    }

    /** PDF tek sayfası ile özdeş çizim (pt uzayı). */
    private static void drawFormPage(Canvas canvas, OptikForm form, List<OptikFormAlan> alanlar,
                                     int pdfW, int pdfH) {
        int canvasW = getCanvasWidthDp(form);
        float scale = (float) pdfW / canvasW;
        ...
        canvas.drawText(form.ad != null ? form.ad : "", pdfW / 2f, titleY, namePaint);
    ...
    public static Uri generatePdf(Context context, OptikForm form, List<OptikFormAlan> alanlar) {
        ...
        drawFormPage(page.getCanvas(), form, alanlar, pdfW, pdfH);
